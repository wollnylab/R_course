...
The following exam is build on the so-called `iris` dataset, which is pre-installed in every R environment. You should be able to answer all questions without knowing the data. But in order to give you an idea what the data looks like, here are the first 5 rows:
...
C.) `select(c(Sepal.Length,Sepal.Width,Petal.Length,Petal.Width))`\
D.) `select(-c(Sepal.Length,Sepal.Width,Petal.Length,Petal.Width))`
...
If you execute the code: `left_join(df1, df2, by = "ID")` on the dataframes **df1** and **df2** as described in the question before, how many *NA* values will the resulting table have in total?
